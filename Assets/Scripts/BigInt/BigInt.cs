...
namespace BigInt
{
	...
	internal class DigitsArray
	{
		...
		{
			get { return (_data[_data.Length - 1] & HiBitSet) == HiBitSet; }
		}

		internal string GetDataAsString()
		{
			string result = "";
			foreach (DType data in _data) {
				result += data + " ";
			}
			return result;
		}
		...
		internal void ResetDataUsed()
		{
			...
			if (IsNegative)
			{
				while (_dataUsed > 1 && _data[_dataUsed - 1] == AllBits)
				{
					--_dataUsed;
				}
				_dataUsed++;
			...
			_dataUsed = System.Math.Min(_data.Length, _dataUsed);
		}
		...
		{
			if (shiftCount == 0) return _data.Length;

			List<DType> temporary = new List<DType>(_data);
			int shiftAmount = DigitsArray.DataSizeBits;
			...
			_data = new DType[temporary.Count];
	...
	public class BigInteger
	{
		...
		public BigInteger(long number)
		{
			...
			{
				_digits[_digits.DataUsed] = (DType)(number & DigitsArray.AllBits);
		...
		private void ConstructFrom(byte[] array, int offset, int length)
		{
			...
			{
				_digits[j] = (DType)((array[i - 3] << 24) + (array[i - 2] << 16) + (array[i - 1] <<  8) + array[i]);
				_digits.DataUsed++;
			}
		...
		{
			int size = System.Math.Max(leftSide._digits.DataUsed, rightSide._digits.DataUsed);
		...
		public static BigInteger operator * (BigInteger leftSide, BigInteger rightSide)
		{
			...
			for (int i = 0; i < leftSide._digits.DataUsed; i++)
			{
				...
				if (carry != 0)
				{
					da[i + rightSide._digits.DataUsed] = (DType)carry;
				}
			}
		...
		private static void MultiDivide(BigInteger leftSide, BigInteger rightSide, out BigInteger quotient, out BigInteger remainder)
		{
			...
			DType val = rightSide._digits[rightSide._digits.DataUsed - 1];
			int d = 0;
			...
			ulong secondDivisor = (rightSide._digits.DataUsed < 2 ? (DType)0 : rightSide._digits[rightSide._digits.DataUsed - 2]);

			int divisorLen = rightSide._digits.DataUsed + 1;
			DigitsArray dividendPart = new DigitsArray(divisorLen, divisorLen);
			DType[] result = new DType[leftSide._digits.Count + 1];
			int resultPos = 0;
		...
		private static void SingleDivide(BigInteger leftSide, BigInteger rightSide, out BigInteger quotient, out BigInteger remainder)
		{
			...
			DType[] result = new DType[leftSide._digits.Count];
		...
		public static BigInteger operator & (BigInteger leftSide, BigInteger rightSide)
		{
			...
			{
				da[idx] = (DType)(leftSide._digits[idx] & rightSide._digits[idx]);
			}
			return new BigInteger(da);
		}
		...
		public static BigInteger operator ~ (BigInteger leftSide)
		{
			...
			{
				da[idx] = (DType)(~(leftSide._digits[idx]));
			}
		...
		public string GetDataAsString() {
			return this._digits.GetDataAsString ();
		}
